...
class MWritersPerNClusterSampler(Sampler):
    ...
    def __init__(self, clusters, writers, pages, m, n, batch_size, length_before_new_iter=100000):
        ...
        # Filter out writers that do not have at least 2 pages in any cluster and at least n clusters
        self.writer_to_cluster_to_page_to_indices = {
            writer: cluster_dict
            for writer, cluster_dict in self.writer_to_cluster_to_page_to_indices.items()
            if any(len(pages) >= 2 for pages in cluster_dict.values()) and len(cluster_dict) >= self.n_per_cluster
        }
        ...
        for _ in range(num_iters):
            ...
            for writer in curr_writer_set:
                ...
                for cluster in clusters[:num_clusters]:
                    ...
                    if len(indices) >= self.m_per_writer:
                        selected_indices = np.random.choice(indices, size=self.m_per_writer, replace=False)
