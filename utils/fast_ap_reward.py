...
class FastAPReward(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple, ref_emb, ref_labels):
        ...
        if torch.sum(safe_H) > 0:
            ...
            FastAP[safe_N] = FastAP[safe_N] / N_pos[safe_N]
...
def compute_loss(self, embeddings, labels, indices_tuple, ref_emb, ref_labels):
        ...
        FastAP_list = []
        for I_pos in [first_I_pos, remaining_I_pos]:
            ...
            if torch.sum(safe_H) > 0:
                ...
                FastAP = torch.sum(FastAP, dim=1)
                FastAP[safe_N] = FastAP[safe_N] / N_pos[safe_N]
                FastAP = 3*(1 - FastAP) * miner_weights #[safe_N]
                FastAP_list.append(FastAP)

        #breakpoint()
        if len(FastAP_list) == 2:
            FastAP = FastAP_list[0] - FastAP_list[1]
